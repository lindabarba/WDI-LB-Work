...
- **Using CSS**: This is the approach that `create-react-app` sets us up with. Notice there is a CSS file, **App.css**, that is imported into the **App.js** component module.
...
Even though we don't intend to use any Bootstrap classes in Mastermind, let's link it in for giggles.
...
Checking our app, we can see Bootstrap is loaded because our header is now jacked up due to Bootstrap setting the CSS `box-sizing` property to `border-box` instead of the default of `content-box`.
...
In React, we use the `className` prop to set class(es) on a DOM component. **Why doesn't React use `class` instead of `className`?** If you inspect the elements in DevTools though, you'll see that the `className` prop does indeed result in a `class` attribute being added to the DOM element.
...
- You can also import CSS for application-wide CSS rules - they're not just for components. General purpose CSS modules could be imported by any component that needs those styles.
- Importing CSS files require tooling. In a React app started with the `create-react-app` CLI, Webpack has been configured to process and import CSS modules.

>**KEY POINT:** Just because we name a spreadsheet for a component and import into that single component, those styles are concatenated and visible application-wide. This is just like when multiple external stylesheets are loaded in an app. So, it's important to prevent rules from conflicting and overriding each other...
...
1. Create a CSS stylesheet file named **GuessRow/GuessRow.css**.
...
3. Create a class named **GuessRow** within **GuessRow.css**:
	...
	```
	>**flexbox** has been recently added to CSS3. It's an amazing way to improvement to layout - once you start using them you will never float another `<div>` again. This lesson will discuss a few of the basics, but I encourage you to checkout the article referenced by this lesson and in today's lab.
...
Later, when the `<div>`s for the individual scores are sized and styled, we will need to come back and further adjust the width, etc.
...
Contrary to what we've been told about avoiding styling elements inline (using the `style` attribute), React actually encourages it.

Inline styling in React uses the `style` prop, however, unlike the `style` attribute on an HTML element, we assign a JS object instead.
...
```
Yup, the objects we use for inline styling are plain 'ol JS object, where:
...
Now we want to apply one of three additional style objects depending upon the score ('P', 'A' or 'I').

An elegant solution could be to first define the style objects within another object, using the score as keys like this:
...
```
`Object.assign` merges the properties of one or more objects into a target object (the first argument) and returns that target object. Note that we are creating a new object as the target, which is then returned after the `baseStyle` and `pegStyles[score]` objects are merged.
...
The approach you choose to style your components is up to you (or your future mentor/boss).
...
- **Inline styling defines styles using JS ______s. objects**
- **What is the name of the prop used to style inline? style**
- **When using CSS Stylesheets, it's important that class names be _______. - unique**
- **What styling approach is best to use if you need to compute styles dynamically? inline**
